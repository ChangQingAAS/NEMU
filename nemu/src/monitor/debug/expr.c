...
uint32_t expr(char *e, bool *success)
{
	...
	{ //只有一个符号时没必要区分
		...
		{
			if (tokens[i].type == '-' && (i == 0 || tokens[i - 1].type == TOKEN_LB || tokens[i - 1].type == TOKEN_NEG || tokens[i - 1].type == '-' || tokens[i - 1].type == TOKEN_ADD || tokens[i - 1].type == '*' || tokens[i - 1].type == TOKEN_RB))
